...
**1. Tools are small, dumb, independent scripts — a stable name over a swappable implementation.**
The tool's *name* is the interface the brain depends on; what sits behind it — a `curl`, a DB
query, a stubbed no-op, a different backend tomorrow — can be replaced without touching the brain or
the skill. `notify` sends a notification. `read_news` reads. They don't know about each other.
Together, all of them are your **toolbox** (the catalog).
...
1. **도구 = 작고 멍청하고 독립적인 스크립트 — 교체 가능한 구현 위에 놓인 안정적 이름(인터페이스).**
   도구의 *이름*이 두뇌가 의존하는 인터페이스이고, 그 뒤의 구현(`curl`이든 DB 쿼리든, 아무것도 안
   하는 stub이든, 내일 다른 백엔드든)은 두뇌나 스킬을 건드리지 않고 갈아끼울 수 있다. `notify`는
