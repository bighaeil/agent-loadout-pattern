# Hand Over the Wheel: A Toolbox Pattern for Autonomous LLM Routines

> Most LLM integrations bolt a model *into* your code. This is about the opposite: letting the
> model **drive** your system, equipped — on its own initiative — from a self-describing toolbox.
> The system stops being the program that calls an LLM, and becomes the *suit* the LLM wears.
...
and do. So the central question of the whole system becomes: *how do we equip the brain well, and
let it reach for the right tool at the right moment?*
...
## The pattern: a self-describing toolbox + mission-only skills
...
name. `notify` sends a notification. `read_news` reads. They don't know about each other.
...
**3. A `loadout` assembler hands the brain its toolbox.** Given a list of tool *names*, it prints
their self-descriptions. This is what the routine runs at wake — it *downloads its toolbox*.

```bash
#!/usr/bin/env bash
# loadout.sh <tool> [tool...] — print the self-descriptions of the named tools
set -euo pipefail
...
**4. The skill becomes *mission only*.** It states what to do and lists its loadout by name. The
descriptions arrive *with the toolbox*, not re-written in the skill:
...
stable and shared). A new routine lists tool names and gets the descriptions for free. Change a
URL and you edit one tool.
...
point of the toolbox, made observable.
...
slots) or it rejects code that doesn't conform. To enforce this toolbox structure at **runtime**,
...
self-describing, observable toolbox is precisely how the brain *takes the wheel*: it wakes, downloads
the tools it's allowed, sees what it can do, and acts — and you can watch it do so at the interface,
not by guessing from side effects. The system stops being a program that occasionally calls a model,
and becomes a suit a model wears.

## Recipe (TL;DR)

1. **Tools** = small scripts, one capability each, mechanics hidden. Add a `--describe` line and a
   boundary log (`INVOKED` + `OK/DRY/ERR`). Side-effecting tools support a `DRY_RUN`.
2. **Loadout** = an assembler that prints the named tools' self-descriptions. The routine runs it
   at wake to *download its toolbox*.
...
> **운전**하게 하고, 스스로 필요할 때 *자기서술 도구상자*에서 장비를 꺼내 입게 하는 것.
> 시스템은 'LLM을 호출하는 프로그램'이 아니라, **LLM이 입는 슈트**가 된다.
...
당신의 모든 엔지니어링은 *슈트*로 간다 — 두뇌가 무엇을 감각·기억·실행할 수 있는가. 그래서 시스템의
핵심 질문은 이거다: *두뇌를 어떻게 잘 입히고, 적시에 맞는 도구를 잡게 할 것인가?*
...
## 패턴: 자기서술 도구상자 + 미션만 있는 스킬

**인터페이스**와 **구현**의 경계를 따라 시스템을 가른다.

1. **도구 = 작고 멍청하고 독립적인 스크립트.** 각자 mechanics 하나를 이름 뒤에 숨긴다. `notify`는
   알림만, `read_news`는 읽기만. 서로 모른다.
2. **도구가 스스로를 설명한다.** `--describe` 한 줄이 "이 도구가 무엇인가"의 단일 출처다. 스킬도
   위키도 아닌, 도구 자신.
3. **`loadout` 조립기가 두뇌에 도구상자를 건넨다.** 도구 *이름* 목록을 주면 그 자기서술을 모아
   출력한다. 루틴은 깰 때 이걸 실행해 *도구상자를 내려받는다*.
...
결과: 스킬은 (자주 바뀌는) 판단을, 도구상자는 (안정적이고 공유되는) 능력을 담는다. 새 루틴은 도구
이름만 적으면 설명이 따라오고, URL이 바뀌면 도구 하나만 고친다.
...
동작한다 — 두뇌는 *구현*이 아니라 *인터페이스*에 의존하니까. 그게 도구상자의 전부이며, 그걸 눈에
보이게 만든 것이다.
...
역전하거나(자기가 돌며 당신 코드를 슬롯에서 호출), 비순응 코드를 거부한다. 이 도구상자 구조를
...
발견하고, 잡고, *사용하는 게 관측되는* 도구. 자기서술적이고 관측 가능한 도구상자가 바로 두뇌가
*핸들을 잡는* 방식이다 — 깨어나 허용된 도구를 내려받고, 할 수 있는 걸 보고, 행동한다. 그리고 당신은
부작용으로 추측하는 게 아니라 인터페이스에서 그걸 지켜본다.

## 레시피 (요약)

1. **도구** = 작은 스크립트, 능력 하나씩, mechanics 숨김. `--describe` + 경계 로그(`INVOKED` +
   `OK/DRY/ERR`). 부작용 도구는 `DRY_RUN` 지원.
2. **로드아웃** = 명명한 도구의 자기서술을 출력하는 조립기. 루틴이 깰 때 실행해 *도구상자를 내려받음*.
